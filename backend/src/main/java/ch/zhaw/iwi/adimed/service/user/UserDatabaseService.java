...
import ch.zhaw.iwi.adimed.model.user.User_;
...
public class UserDatabaseService extends AbstractCrudDatabaseService<User, Long> {
	...
	protected void orderBy(Root<User> root, List<Order> orderList) {
		orderList.add(getCriteriaBuilder().asc(root.get(User_.lastName)));
		orderList.add(getCriteriaBuilder().asc(root.get(User_.firstName)));
	}
	...
	public class EMailFilter extends AbstractCrudDatabaseService<User, Long>.AbstractListFilter {
		...
		public void appendFilter(CriteriaQuery<User> criteriaQuery, Root<User> root) {
			...
			addPredicate(criteriaQuery, cb.equal(cb.lower(root.get(User_.email)), email));
		}
	}
